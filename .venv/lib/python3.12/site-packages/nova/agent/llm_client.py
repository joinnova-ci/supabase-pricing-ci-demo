...
class LLMClient:
    ...
    def complete(self, system: str, user: str, temperature: float = 0.3, max_tokens: int = 100000) -> str:
        """
        ...
        try:
            if not self.provider:
                raise ValueError("LLM provider not initialized. Check that openai or anthropic packages are installed.")
            ...
            if elapsed > self.settings.llm_call_timeout_sec and self.verbose:
    ...
    def _complete_openai(self, system: str, user: str, temperature: float, max_tokens: int) -> str:
        ...
        try:
            ...
            if "gpt-5" in self.model.lower():
                ...
                # Set reasoning effort from settings
                kwargs["reasoning_effort"] = getattr(self.settings, 'default_reasoning_effort', 'medium')
            else:
            ...
            if content:
                ...
                if self.verbose:
                    print("[Nova Debug - LLM] WARNING: OpenAI returned None/empty content!")
                content = ""
            return content
        ...
        try:
            ...
            if response.content and len(response.content) > 0:
                ...
                if content:
                    ...
                    if self.verbose:
                        print("[Nova Debug - LLM] WARNING: Anthropic returned None/empty text!")
                    content = ""
            else:
                if self.verbose:
                    print("[Nova Debug - LLM] WARNING: Anthropic returned empty content array!")
                content = ""
            return content
        except Exception as e:
            if self.verbose:
