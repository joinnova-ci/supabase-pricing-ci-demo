...
def build_strict_critic_prompt(patch: str, failing_tests: List[Dict[str, Any]], 
                             num_original_failures: int, test_results_after_patch: Optional[str] = None) -> str:
    """
...
"""
    ...
    if test_results_after_patch:
        prompt += f"""

ACTUAL TEST RESULTS AFTER APPLYING PATCH:
```
{test_results_after_patch[:1000]}{'...(truncated)' if len(test_results_after_patch) > 1000 else ''}
```

CRITICAL: Review the ACTUAL test results above. If tests are still failing, you MUST reject!
"""
