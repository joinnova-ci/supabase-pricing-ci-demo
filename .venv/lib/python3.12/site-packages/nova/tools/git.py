...
- Minimal diff; no runtime artifacts committed

Note: CI summary & JUnit attached as workflow artifacts"""
        ...
        if changed_files is not None:
            ...
            # Only add source files that were actually modified (whitelist approach)
            ...
            if success and output.strip():
                ...
                # Whitelist: only Python source files and essential config files
                files_to_add = []
                for f in modified_files:
                    # Add Python files (but not in __pycache__ or test output dirs)
                    if (f.endswith('.py') and 
                        '__pycache__' not in f and 
                        '.nova/' not in f and
                        'test-results' not in f):
                        files_to_add.append(f)
                    # Add essential config files if modified
                    elif f in ['requirements.txt', 'setup.py', 'pyproject.toml', 'setup.cfg']:
                        files_to_add.append(f)
                
                if files_to_add:
                    success, output = self._run_git_command("add", "--", *files_to_add)
                    if not success:
                        if self.verbose:
                            console.print(f"[red]Failed to stage changes: {output}[/red]")
                        return False
                    ...
                    if self.verbose:
                        console.print("[dim]No source files to stage[/dim]")
            else:
                # Use git add -u to only stage tracked modified files
                success, output = self._run_git_command("add", "-u", "*.py")
                if not success and self.verbose:
                    console.print(f"[yellow]Warning: Could not stage Python files: {output}[/yellow]")
